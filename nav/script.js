...
// 图片拖放上传功能
// dragArea is already declared above, so we'll remove this duplicate declaration
// Get drag and drop area elements
...
function addCategoryToSidebar(category, iconClass = "fas fa-folder") {
  ...
  newLink.innerHTML = `<i class="${iconClass}"></i> ${category}`;
  // Event listener is now handled by event delegation on .category-list

  li.appendChild(newLink);
  categoryList.appendChild(li);

  // No longer need to update global categoryLinks due to event delegation
}
...
function addEventListeners() {
  ...
  if (categoryListElement) {
    categoryListElement.addEventListener("click", function (e) {
      // Check if the clicked element or its parent is an <a> tag within a <li>
      let targetLink = e.target;
      if (targetLink.tagName !== "A") {
        targetLink = targetLink.closest("li > a");
      }
      if (targetLink && this.contains(targetLink)) {
        // Ensure the link is a child of categoryListElement
        // Call filterSitesByCategory, ensuring 'this' context is the link and passing the event
        filterSitesByCategory.call(targetLink, e);
      }
    });
  }
  ...
  // Site forms
  if (addSiteForm) addSiteForm.addEventListener("submit", handleAddSite);
  if (editSiteForm) editSiteForm.addEventListener("submit", handleUpdateSite);
  const removeLinkBtn = document.getElementById("removeLinkBtn");
  if (removeLinkBtn) removeLinkBtn.addEventListener("click", handleRemoveSite);
